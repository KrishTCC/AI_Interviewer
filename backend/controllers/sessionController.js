...
// URL for the Python AI Microservice (override in production)
const AI_SERVICE_URL = process.env.AI_SERVICE_URL || 'http://localhost:8000';

const normalizeQuestionItem = (item, index, codingCount, fallbackTopic = '') => {
    ...
    return {
        questionText: item.questionText || item.question || item.prompt || 'Explain the concept in detail.',
        questionType: item.questionType || (index < codingCount ? 'coding' : 'oral'),
        idealAnswer: item.idealAnswer || item.ideal_answer || '',
        topic: item.topic || fallbackTopic,
        conceptTags: item.conceptTags || item.concept_tags || [],
...
const createSession = asyncHandler(async (req, res) => {
    ...
    if (interviewMode === 'resume' && !req.file) {
        res.status(400);
        throw new Error('Please upload a resume file for resume-based interview mode.');
    }
    ...
    (async () => {
        const uploadedResumePath = req.file ? path.join(process.cwd(), req.file.path) : null;
        try {
            ...
            if (interviewMode === 'resume' && hasResumeFile) {
                ...
                formData.append('resume_file', fs.createReadStream(path.join(process.cwd(), req.file.path)));

                aiResponse = await fetch(`${AI_SERVICE_URL}/generate-resume-questions`, {
                    method: 'POST',
                    body: formData,
                    headers: formData.getHeaders(),
                });
            } else {
                aiResponse = await fetch(`${AI_SERVICE_URL}/generate-questions`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        mode: interviewMode === 'company' ? 'company' : 'general',
                        company,
                        role,
                        level,
                        difficulty,
            ...
            const questionsArray = (aiData.questions || []).map((item, index) => {
                const normalized = normalizeQuestionItem(item, index, codingCount, topic);
                return {
                    questionText: normalized.questionText,
                    questionType: normalized.questionType,
                    idealAnswer: normalized.idealAnswer || 'pending',
            ...
            session.company = aiData.company || company || '';
            session.interviewMode = aiData.mode || interviewMode || 'general';
            session.resumeSummary = aiData.resume_profile?.experience_summary || '';
            session.topic = aiData.resume_profile?.target_topics?.[0] || topic || '';
            await session.save();
...
const evaluateAnswerAsync = async (io, userId, sessionId, questionIndex, audioFilePath = null, code = null) => {
    ...
    try {
        ...
        question.semanticSimilarity = evalData.semanticSimilarity || 0;
        question.missingConcepts = evalData.missingConcepts || [];
