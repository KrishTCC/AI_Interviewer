...
const COMPANIES = ["Amazon", "Google", "Microsoft", "Meta", "Netflix", "Apple"];
const INTERVIEW_MODES = [
  { label: "General Technical", value: "general" },
...
const Dashboard = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    company: COMPANIES[0],
    interviewMode: INTERVIEW_MODES[0].value,
    level: LEVELS[0],
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    if (formData.interviewMode === 'resume') {
      if (!resumeFile) {
        toast.error('Please upload your resume before starting a resume-based interview.');
        return;
      }
      ...
      payload.append('interviewMode', formData.interviewMode);
      payload.append('difficulty', formData.level);
    ...
    dispatch(createSession({
      ...
      company: formData.interviewMode === 'company' ? formData.company : '',
    }));
  }
      ...
      <div className="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-xl sm:shadow-2xl shadow-slate-200 border border-slate-100 overflow-hidden">
        ...
        <form onSubmit={onSubmit} className="p-6 sm:p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6 items-end">
          <div className="space-y-1.5 md:col-span-2 lg:col-span-5">
            ...
            <select name="interviewMode" value={formData.interviewMode} onChange={onChange} className="w-full bg-slate-50 border-none rounded-xl sm:rounded-2xl p-3 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-teal-500">
              {INTERVIEW_MODES.map((mode) => <option key={mode.value} value={mode.value}>{mode.label}</option>)}
            </select>
          </div>

          {formData.interviewMode === 'company' && (
            <div className="space-y-1.5 md:col-span-2 lg:col-span-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Company</label>
              <select name="company" value={formData.company} onChange={onChange} className="w-full bg-slate-50 border-none rounded-xl sm:rounded-2xl p-3 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-teal-500">
          ...
          {formData.interviewMode === 'general' && (
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Focus Topic</label>
              <select name="topic" value={formData.topic} onChange={onChange} className="w-full bg-slate-50 border-none rounded-xl sm:rounded-2xl p-3 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-teal-500">
                {TECH_TOPICS.map((topic) => <option key={topic} value={topic}>{topic}</option>)}
              </select>
            </div>
          )}

          {formData.interviewMode === 'resume' && (
            <div className="space-y-1.5 md:col-span-2 lg:col-span-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Resume File</label>
              <input type="file" accept=".pdf,.doc,.docx,.txt" onChange={onResumeChange} className="w-full bg-slate-50 border-none rounded-xl sm:rounded-2xl p-3 text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-teal-500" />
            </div>
          )}
