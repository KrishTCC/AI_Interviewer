...
function SessionReview() {
    ...
    const { overallScore, metrics, role, company, interviewMode, level, questions, startTime, endTime } = activeSession;
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-100 pb-6 sm:pb-10">
                <div>
                    ...
                    {(company || interviewMode) && (
                        <p className="mt-3 text-xs sm:text-sm font-bold text-slate-500 uppercase tracking-[0.2em]">
                            {interviewMode ? interviewMode.replace(/-/g, ' ') : ''}{company ? ` • ${company}` : ''}
                        </p>
                    )}
                </div>
            </div>
                            ...
                            <div className="p-6 sm:p-10 space-y-6 sm:space-y-8">
                                ...
                                <div className="flex flex-col lg:flex-row justify-between items-start gap-4 sm:gap-6">
                                    ...
                                    <div className="flex flex-wrap gap-2 shrink-0">
                                        ...
                                        <div className="px-3 py-1.5 sm:px-5 sm:py-2 rounded-xl sm:rounded-2xl border border-violet-100 bg-violet-50 flex items-center gap-2">
                                            <span className="text-[8px] sm:text-[10px] font-black uppercase text-slate-400">Sim</span>
                                            <span className="text-xs sm:text-sm font-black text-violet-600">{Math.round((q.semanticSimilarity || 0) * 100)}%</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10 pt-6 sm:pt-8 border-t border-slate-50">
                                    <div className="space-y-3">
                                        ...
                                        {Array.isArray(q.missingConcepts) && q.missingConcepts.length > 0 && (
                                            <div className="flex flex-wrap gap-2 pt-2">
                                                {q.missingConcepts.map((concept) => (
                                                    <span key={concept} className="text-[10px] sm:text-xs font-bold px-3 py-1 rounded-full bg-rose-50 text-rose-600 border border-rose-100">
                                                        Missing: {concept}
                                                    </span>
                                                ))}
                                            </div>
